...
Cohesion: 0.06
Nodes (46): build_client_env(), write_env_file(), bootstrap(), _build_extra_env(), _configure_logging(), _detect_platform(), kb_ingest(), kb_query() (+38 more)
...
Cohesion: 0.22
Nodes (15): run_safe_command, _check_precice_cli(), _load_topology_schema(), precice_config_check, precice_config_format, precice_config_visualize, precice_init, precice_profiling_analyze (+7 more)
...
Cohesion: 0.43
Nodes (4): GenericPlatform, GenericPlatform, Print the MCP server JSON snippet for manual integration., Print the MCP server JSON snippet for manual integration.
...
Cohesion: 0.33
Nodes (6): 1. Confusing `requirements.txt` with the real package definition, 2. Forgetting that the server is launched by the client, not by the CLI, 3. Testing only from the repo root, 4. Breaking the safety model while adding convenience, 5. Assuming the whole KB is fetched at query time, Common Contributor Pitfalls
...
Cohesion: 0.50
Nodes (4): Embedding configuration, Environment Variables In Detail, GitHub integration, Project and KB paths
